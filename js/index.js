...
$('#submitBtn').click(async function () {
  ...
  var random  = name
  var randomletter  = random.charAt(0)
  reggame = await contractCall('register', [name, phone, email, price, work, availability, company, multihash], 0)
...
$("#section").on( "click", ".downloadjobSample", async function (event) {
  ...
  console.log("Downloading Chainee jobSample ")
  dataIndex = event.target.id
  console.log("dataindex", dataIndex)
  jobSample = await callStatic('getChaineeById', [dataIndex])
  console.log("jobSample Link")
